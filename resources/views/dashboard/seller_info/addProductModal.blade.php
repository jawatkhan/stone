    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="form-horizontal" role="form" action="" method="GET" id="product-Add">
                    {{ csrf_field() }}
                    @section('editmethod')
                    @show
...
var finalEnlishToBanglaNumber={'0':'০','1':'১','2':'২','3':'৩','4':'৪','5':'৫','6':'৬','7':'৭','8':'৮','9':'৯'};
    ...
    $("#p_rate,#qty").keyup(function(){
        ...
        var total = rate*qty;
        console.log(total);
        $("#total_amount").val(total.toFixed().getDigitBanglaFromEnglish());
    })
...
// ------------add Product-----------
    $('body').on('click', '#addProductForm', function(){
        ...
        $.ajax({
            type:'GET',
            url:'{!!URL::to('/seller-product-add-temp')!!}',
            data:formData,
            success:function(data) {
               ...
               console.log(data);
                ...
                if(data.errors) {                  
                    if(data.errors.article_no){
                        $( '#article_no_error' ).html('The Category name field is required.');
                    }
                    if(data.errors.color_id){
                        $( '#color_error' ).html('The Product name field is required.');
                    }
                    if(data.errors.qty){
                ...
                for(var i=0;i<valcost.length;i++){
                    var owner = (valcost[i].owner_id==1) ? "@lang('form.own')" : "@lang('form.buyer')";
...
// ------------ add extra cost--------------------------
    $('body').on('click', '#extraCostSubmit', function(){
        ...
        $.ajax({
            type:'GET',
            url:'{!!URL::to('/seller-extra-cost-add')!!}',
            data:formData,
            success:function(data) {
...
// ------------ update product------------------
    ...
    $('body').delegate('#editProduct','click',function(e){
        ...
        $.ajax({
                type:'get',
                url:'{!!URL::to('/seller-product-edit-temp')!!}',
                data:{id:id},
                success:function(data){
                    console.log('success');
                    console.log(data);
                    console.log(data.id);
                    $('#product-edit').find('#id').val(data.id);
                    $('#product-edit').find('.article_no').val(data.article_no).attr('selected','true');
                    // $('#product-edit').find('.filter-option').html(data.color_id);
                    $('#product-edit').find('.color_id').val(data.color_id).attr('selected','true');

                    // $('#editextraCost').find('#checkOwner1').attr('checked','true');
                    // $('#editextraCost').find('#checkOwner2').attr('checked','true');
                    $('#product-edit').find('#qty').val((data.qty).toFixed().getDigitBanglaFromEnglish());
                    $('#product-edit').find('#p_rate').val((data.rate).toFixed().getDigitBanglaFromEnglish());
                    $('#product-edit').find('#total_amount').val((data.amount).toFixed().getDigitBanglaFromEnglish());
                    $('#product-edit').find('.date').val(data.date);
                },
                error:function(){
    ...
    $('body').on('click', '#editProductSubmit', function(){
        var UpdateProductForm = $("#product-edit");
        ...
        $.ajax({
            type:'GET',
            url:'{!!URL::to('/seller-product-update-temp')!!}',
            data:formData,
            success:function(data) {
    ...
    $('body').on('click','#pro-dell',function(){
        ...
        $.ajax({
            type:'GET',
            url:'{!!URL::to('/seller-product-delete')!!}',
    ...
    $('body').on('click','#cost-dell',function(){
        ...
        $.ajax({
            type:'GET',
            url:'{!!URL::to('/seller-extra-cost-delete')!!}',
            data:{id:id,invoice_id:invoice_id,client_id:client_id},
            dataType:'json',
            success:function(data){
                console.log(data);
    ...
    // --------- Update Extra Cost-----------------------
        ...
        $.ajax({
                type:'get',
                url:'{!!URL::to('/seller-extra-cost-edit')!!}',
                data:{id:id},
                success:function(data){
                    //console.log('success');
                    // console.log(data);
    ...
    $('body').on('click', '#updateExtraCostSubmit', function(){
        ...
        $.ajax({
            type:'GET',
            url:'{!!URL::to('/seller-extra-cost-update')!!}',
            data:formData,
            success:function(data) {
...
// ---------- Final Submit Voucher----------------
    $('body').on('click','#finalVoucherSubmit',function(){
        var invoice_id = $('#invoice_id').val();
        var client_id = $('#client_id').val();
        ...
        $.ajax({
            type:'GET',
            url:'{!!URL::to('/seller-final-voucher-create')!!}',
            ...
            success:function(data){
                ...
                console.log(data);
                if(data.success){
                    window.location = '{!!URL::to('/seller-voucher-page')!!}'+'/'+data.client_id+'/'+data.invoice_id;
                }else{
                    alert(data.message);
                }
            }
