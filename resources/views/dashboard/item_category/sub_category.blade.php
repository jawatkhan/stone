@extends('layouts.app')
@section('page_title',__('general.sub-category'))
@section('content')
...
<div class="panel-body">
	<form class="form-horizontal" action="{{ URL::to('/sub-category') }}/{{$id}}" method="post">
		{{ csrf_field() }}
		@section('editmethod')
		@show
		<div class="form-group hidden">
          	<label for="">@lang('form.id') : </label>
            <input type="text" class="form-control" name="categoryid" value="{{$id}}">
        </div>
		<div class="form-group {{ $errors->has('category_name') ? ' has-error' : '' }}">
            <label class="col-lg-3 control-label">@lang('form.category-name') :</label>
            <div class="col-lg-8">
              <input class="form-control" value="@yield('category_name')" type="text" name="category_name">
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    @foreach ($braedCumb as $parent)
    <li class="breadcrumb-item"><a href="{{ URL::to('/sub-category') }}/{{$parent->category_id}}">{{$parent->category_name}}</a></li>
  	@endforeach
  </ol>
</nav>
		...
		<tbody>
			  ...
			  @foreach ($categories as $category)
	      		<tr>
	      			...
	      			<td style="width: 15%;">
	      				<a href="{{URL::to('/sub-category-edit')}}/{{$category->parent_id}}/{{$category->id}}" class="btn btn-primary btn-lg"><i class="fa fa-pencil-square-o" aria-hidden="true"> @lang('button.edit')</i></a>
