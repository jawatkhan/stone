...
class Library
{
	// --------- BanglaToEnglish and EnglishToBangla number Converter--------------
    ...
    public static function engToBn($number){
        $bn_number = strtr($number,self::$eng_to_bn);
        return $bn_number;
    }

    public static function numToWord($number){
        ...
        if(is_float($number)){
            $dot = explode(".", $number);
            return (new static)->numberSelector($dot[0]).' দশমিক '.(new static)->numToBnDecimal($dot[1]);
        }else{
            return (new static)->numberSelector($number);
        }

    }
    public static function numToBn($number){
        $word = strtr($number,self::$num_to_bd);
        return $word;
    }
    public static function numToBnDecimal($number){
        $word = strtr($number,self::$num_to_bn_decimal);
        return $word;
    }

    public static function numberSelector($number){
        if($number > 9999999){
            return (new static)->crore($number);
        }elseif($number > 99999){
            return (new static)->lakh($number);
        }elseif($number > 999){
            return (new static)->thousand($number);
        }elseif($number > 99){
            return (new static)->hundred($number);
        }else{
            return (new static)->underHundred($number);
        }
    }

    public static function underHundred($number){
        $number = ($number == 0)?'': (new static)->numToBn($number);
        return $number;
    }
    ...
    public static function hundred($number){
        $a = (int)($number/100);
        $b = $number%100;
        $hundred = ($a == 0)?'': (new static)->numToBn($a).' '.self::$hundred;
        return $hundred.' '.(new static)->underHundred($b);
    }

    public static function thousand($number){
        $a = (int)($number/1000);
        $b = $number%1000;
        $thousand = ($a == 0)?'': (new static)->numToBn($a).' '.self::$thousand;
        return $thousand.' '.(new static)->hundred($b);
    }

    public static function lakh($number){
        $a = (int)($number/100000);
        $b = $number%100000;
        $lakh = ($a == 0)?'': (new static)->numToBn($a).' '.self::$lakh;
        return $lakh.' '.(new static)->thousand($b);
    }

    public static function crore($number){
        $a = (int)($number/10000000);
        $b = $number%10000000;
        $more_than_core = ($a>99)?(new static)->lakh($a):(new static)->numToBn($a);
        return $more_than_core.' '.self::$crore.' '.(new static)->lakh($b);
    }
